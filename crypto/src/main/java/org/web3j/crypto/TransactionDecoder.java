...
public class TransactionDecoder {
	...
	public static RawTransaction decode(String hexTransaction) {
		...
		if (values.getValues().size() == 6
				|| (values.getValues().size() == 8 && ((RlpString) values.getValues().get(7)).getBytes().length == 10)
			...
			final byte[] r = Numeric.toBytesPadded(Numeric.toBigInt(((RlpString) values.getValues().get(7)).getBytes()),
					32);
			final byte[] s = Numeric.toBytesPadded(Numeric.toBigInt(((RlpString) values.getValues().get(8)).getBytes()),
					32);
			...
			return new SignedRawTransaction(nonce, gasPrice, gasLimit,
					Bech32.addressEncode(NetworkParameters.getHrp(getChainId(v)), to), value, data, signatureData);
		}
	}
