...
public class Sign {
	...
	public static SignatureData signMessage(byte[] message, ECKeyPair keyPair) {
		...
		if (keyPair.getAlgorithm() == ECAlgorithm.Secp256k1)
			return signMessage(message, keyPair, true);
		else
			return signMessageSm2(message, keyPair, true);
	}

	public static SignatureData signMessage(byte[] message, ECKeyPair keyPair, boolean needToHash) {
		if (keyPair.getAlgorithm() != ECAlgorithm.Secp256k1) {
			throw new RuntimeException("This method only supports signing using the secp256k1 algorithm");
		}
	...
	public static SignatureData signMessageSm2(byte[] message, ECKeyPair keyPair, boolean needToHash) {
		if (keyPair.getAlgorithm() != ECAlgorithm.SM2) {
			throw new RuntimeException("This method only supports signing using the sm2 algorithm");
		}

		byte[] messageHash;
		if (needToHash) {
			messageHash = SM3Utils.sm3(message);
		} else {
			messageHash = message;
		}

		byte[] signData = keyPair.sm2Sign(messageHash);

		byte[] r = Arrays.copyOfRange(signData, 0, 32);
		byte[] s = Arrays.copyOfRange(signData, 32, 64);

		int headerByte = signData[64] + 27;
		byte[] v = new byte[] { (byte) headerByte };
	 ...
	 * 
	 * <p>
	 * This method only supports the use of secp256k1 algorithm.
	 * If using sm2 algorithm, @see SM2Utils.recoverPubKey(byte[] msg, byte[] sign)
	 * </p>
	 *
	 ...
	 *
	 *<p>This method only supports the use of secp256k1 algorithm.</p>
	 *
	 ...
	 * public key to determine if the signature was correct.
	 * 
	 * <p>This method only supports the use of sm2 algorithm.</p>
	 * 
	 * @param message  RLP encoded message.
	 * @param signatureData The message signature components
	 * @return the public key used to sign the message
	 * @throws SignatureException SignatureException If the public key could not be recovered or if there was a
	 *     signature format error.
	 */
	public static BigInteger signedMessageToKeySm2(byte[] message, SignatureData signatureData)
		...
		System.arraycopy(signatureData.getR(), 0, sign, 0, 32);
		System.arraycopy(signatureData.getS(), 0, sign, 32, 32);
		sign[64] = (byte) v;

		byte[] messageHash = SM3Utils.sm3(message);

		byte[] pubKey = SM2Utils.recoverPubKey(messageHash, sign);
		return Numeric.toBigInt(pubKey);
	}
	 ...
	 * @return BigInteger encoded public key
	 */
	public static BigInteger publicKeyFromPrivateSm2(BigInteger privKey) {
		byte[] pubKey = SM2Utils.pubKeyFromPrivateKey(Numeric.toBytesPadded(privKey, Keys.PRIVATE_KEY_SIZE));
		return Numeric.toBigInt(pubKey);
	}
