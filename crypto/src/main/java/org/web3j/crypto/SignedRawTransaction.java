...
public class SignedRawTransaction extends RawTransaction {
	...
	public SignedRawTransaction(BigInteger nonce, BigInteger gasPrice, BigInteger gasLimit, String to, BigInteger value,
			String data, Sign.SignatureData signatureData) {
	...
	public String getFromSm2() throws SignatureException {
		...
		byte[] s = signatureData.getS();
		Sign.SignatureData signatureDataV = new Sign.SignatureData(getRealV(v), r, s);
		BigInteger key = Sign.signedMessageToKeySm2(encodedTransaction, signatureDataV);

		return Bech32.addressEncode(NetworkParameters.getHrp(chainId), "0x" + Keys.getSm2Address(key));
	}
