...
public class Keys {
	...
	/**
	 * Create a ECKeyPair using SECP-256k1 curve.
	 * 
	 * @return ECKeyPair
	 * 
	...
	/**
	 * Create a ECKeyPair using sm2.
	 * 
	 * @return ECKeyPair
	 */
	public static ECKeyPair createSm2EcKeyPair() {
		return ECKeyPair.createSm2(SM2Utils.generate());
	}
	...
	public static String getAddress(ECKeyPair ecKeyPair) {
		if (ecKeyPair.getAlgorithm() == ECAlgorithm.Secp256k1)
			...
			return getSm2Address(ecKeyPair.getPublicKey());
	}
	...
	public static String getSm2Address(BigInteger publicKey) {
		return getSm2Address(Numeric.toHexStringWithPrefixZeroPadded(publicKey, PUBLIC_KEY_LENGTH_IN_HEX));
	}
	...
	public static String getSm2Address(String publicKey) {
		String publicKeyNoPrefix = Numeric.cleanHexPrefix(publicKey);
		...
		String hash = SM3Utils.sm3(publicKeyNoPrefix);
		return hash.substring(hash.length() - ADDRESS_LENGTH_IN_HEX);
	}
	...
	public static byte[] getSm2Address(byte[] publicKey) {
		byte[] hash = SM3Utils.sm3(publicKey);
		// right most 160 bits
		return Arrays.copyOfRange(hash, hash.length - 20, hash.length);
	}
