...
//        try {
//            PlatoneGasPrice platoneGasPrice = web3j.platoneGasPrice().send();
//            logger.info("platoneGasPrice >>> " + JSON.toJSONString(platoneGasPrice));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
//    }
...
//     *
//     * @Test public void web3ClientVersion() { try { Web3ClientVersion web3ClientVersion =
//     *       web3j.web3ClientVersion().send(); logger.info("web3ClientVersion >>> " +
//     *       JSON.toJSONString(web3ClientVersion)); } catch (IOException e) { e.printStackTrace(); } }
//     * 
//     *       /** Returns Keccak-256 (not the standardized SHA3-256) of the given data.
//     *
//     *       Result :
//     *       {"id":0,"jsonrpc":"2.0","result":"0x0eccfe5d014e8c46d0a4afdd7ba4a75f269473fb60de682768439deadd8e1165"}
//     */
//    @Test
...
//
//    /**
//     * Returns the current platone protocol version.
//     *
//     * Result : {"id":0,"jsonrpc":"2.0","protocolVersion":"0x3f","result":"0x3f"}
//     */
//    @Test
//    public void platoneProtocolVersion() {
//        PlatoneProtocolVersion platonProtocolVersion;
//        try {
//            platonProtocolVersion = web3j.platoneProtocolVersion().send();
...
//
//    /**
//     * Returns an object with data about the sync status or false. Object|Boolean, An object with sync status data or
//     * FALSE, when not syncing: startingBlock: QUANTITY - The block at which the import started (will only be reset,
//     * after the sync reached his head) currentBlock: QUANTITY - The current block, same as platon_blockNumber
...
//        try {
//            PlatoneSyncing platonSyncing = web3j.platoneSyncing().send();
...
//        try {
//            PlatoneAccounts platonAccounts = web3j.platoneAccounts().send();
...
//        try {
//            PlatoneBlockNumber platonBlockNumber = web3j.platoneBlockNumber().send();
...
//        try {
//            PlatoneGetBalance platonGetBalance =
//                web3j.platoneGetBalance(address, DefaultBlockParameterName.LATEST).send();
...
//        try {
//            PlatoneGetStorageAt platonGetStorageAt =
//                web3j.platoneGetStorageAt(address, position, DefaultBlockParameterName.LATEST).send();
...
//        try {
//            PlatoneGetTransactionCount platonGetTransactionCount =
//                web3j.platoneGetTransactionCount(address, DefaultBlockParameterName.LATEST).send();
...
//        /**
//         * QUANTITY|TAG - integer of a block number, or the string "earliest", "latest" or "pending", as in the default
//         * block parameter. Boolean - If true it returns the full transaction objects, if false only the hashes of the
//         * transactions.
//         */
//        try {
//            PlatoneBlock platonBlock = web3j.platoneGetBlockByNumber(DefaultBlockParameterName.LATEST, false).send();
...
//            PlatoneGetBlockTransactionCountByHash platonGetBlockTransactionCountByNumber =
//                web3j.platoneGetBlockTransactionCountByHash(blockHash).send();
//            logger.info("PlatonGetBlockTransactionCountByNumber >>> "
//                + JSON.toJSONString(platonGetBlockTransactionCountByNumber));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
//    }
...
//        try {
//            PlatoneGetBlockTransactionCountByNumber platonGetBlockTransactionCountByNumber =
//                web3j.platoneGetBlockTransactionCountByNumber(DefaultBlockParameterName.EARLIEST).send();
//            logger.info("PlatonGetBlockTransactionCountByNumber >>> "
//                + JSON.toJSONString(platonGetBlockTransactionCountByNumber));
//        } catch (IOException e) {
//            e.printStackTrace();
...
//        try {
//            PlatoneGetCode platonGetCode =
//                web3j.platoneGetCode(contractAddress, DefaultBlockParameterName.LATEST).send();
...
//
//        try {
//            PlatoneSign platonSign = web3j.platoneSign(address, sha3HashOfDataToSign).send();
...
//        String from = address;
//
//        // (optional) Integer of a nonce. This allows to overwrite your own pending transactions that use the same
//        // nonce.
//        BigInteger nonce = null;
//        PlatoneGetTransactionCount platonGetTransactionCount;
//        try {
//            platonGetTransactionCount = web3j.platoneGetTransactionCount(from, DefaultBlockParameterName.LATEST).send();
...
//
//        // (optional, default: 90000) Integer of the gas provided for the transaction execution. It will return unused
//        // gas.
...
//        try {
//            PlatoneSendTransaction platonSendTransaction = web3j.platoneSendTransaction(transaction).send();
...
//        PlatoneSendTransaction platonSendTransaction;
//        try {
//            platonSendTransaction = web3j.platoneSendRawTransaction(hexValue).send();
//            logger.info("platonSendTransaction >>> " + JSON.toJSONString(platonSendTransaction));
//        } catch (IOException e1) {
//            e1.printStackTrace();
//        }
...
//
//        try {
//            PlatoneCall platonCall = web3j.platoneCall(transaction, DefaultBlockParameterName.LATEST).send();
...
//     * transaction will not be added to the blockchain. Note that the estimate may be significantly more than the amount
//     * of gas actually used by the transaction, for a variety of reasons including EVM mechanics and node performance.
//     */
//    @Test
...
//
//        try {
//            PlatoneEstimateGas platonEstimateGas = web3j.platoneEstimateGas(transaction).send();
...
//        String blockHash = "0x667ab25b762d374dc4c2263acb2273f32a8511390d5fc23b5b2bc4dc1164258a";
//        try {
//            PlatoneBlock platonBlock = web3j.platoneGetBlockByHash(blockHash, true).send();
//            logger.info("platonBlock hash >>> " + platonBlock.getBlock().getHash());
...
//        try {
//            PlatoneTransaction platonTransaction =
//                web3j.platoneGetTransactionByBlockHashAndIndex(blockHash, transactionIndex).send();
...
//        try {
//            PlatoneTransaction platonTransaction = web3j.platoneGetTransactionByHash(transactionHash).send();
//            logger.info("platonTransaction >>> " + JSON.toJSONString(platonTransaction));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//        try {
//            PlatoneTransaction platonTransaction =
//                web3j.platoneGetTransactionByBlockNumberAndIndex(new DefaultBlockParameterNumber(blockNumber),
//                    transactionIndex).send();
//            logger.info("platonTransaction >>> " + JSON.toJSONString(platonTransaction));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//        String transactionHash = "0x8c35272a7c95ab38b9cd44180350ea18925220dc7b34d8e01a0167f5b299afec";
//        try {
//            PlatoneGetTransactionReceipt platonGetTransactionReceipt =
//                web3j.platoneGetTransactionReceipt(transactionHash).send();
...
//     * has changed, call platon_getFilterChanges.
//     */
//    @Test
//    public void platonNewFilter() {
//        org.web3j.protocol.core.methods.request.PlatoneFilter platonFilter =
//            new org.web3j.protocol.core.methods.request.PlatoneFilter(DefaultBlockParameterName.EARLIEST,
//                DefaultBlockParameterName.LATEST, contractAddress);
//        try {
//            PlatoneFilter platonFilter_res = web3j.platoneNewFilter(platonFilter).send();
...
//        try {
//            PlatoneFilter platonFilter = web3j.platoneNewBlockFilter().send();
...
//        try {
//            PlatoneFilter platonFilter = web3j.platoneNewPendingTransactionFilter().send();
//            logger.info("platonFilter >>> " + JSON.toJSONString(platonFilter));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//     * timeout when they aren't requested with platon_getFilterChanges for a period of time.
//     */
//    @Test
//    public void platonUninstallFilter() {
//        BigInteger filterId = new BigInteger("258575391212208728421850232718129168292", 10);
//        try {
//            PlatoneUninstallFilter platonUninstallFilter = web3j.platoneUninstallFilter(filterId).send();
...
//        try {
//            PlatoneLog platonLog = web3j.platoneGetFilterChanges(filterId).send();
...
//        BigInteger filterId = new BigInteger("336685864917132318099501088544437319929", 10);
//        try {
//            PlatoneLog platonLog = web3j.platoneGetFilterLogs(filterId).send();
//            logger.info("platonLog >>> " + JSON.toJSONString(platonLog));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//                DefaultBlockParameterName.LATEST, contractAddress);
//        try {
//            PlatoneLog platonLog = web3j.platoneGetLogs(platonFilter).send();
//            logger.info("platonLog >>> " + JSON.toJSONString(platonLog));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//        try {
//            PlatonePendingTransactions platonPendingTransactions = web3j.platonePendingTx().send();
...
//        try {
//            PlatoneEvidences platonEvidences = web3j.platoneEvidences().send();
