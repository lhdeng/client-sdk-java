...
public class ContractTest extends ManagedTransactionTester {
	...
	public void setUp() throws Exception {
		...
		contract = new TestContract(ADDRESS, web3j, getVerifiedTransactionManager(SampleKeys.CREDENTIALS),
				new DefaultGasProvider(), 200L);
	}
	...
	public void testCallSingleValue() throws Exception {
		...
		PlatoneCall ethCall = new PlatoneCall();
	...
	public void testCallMultipleValue() throws Exception {
		...
		assertThat(contract.callMultipleValue().send(),
				equalTo(Arrays.asList(new Uint256(BigInteger.valueOf(55)), new Uint256(BigInteger.valueOf(7)))));
	}
	...
	private void prepareCall(PlatoneCall ethCall) throws IOException {
		Request<?, PlatoneCall> request = mock(Request.class);
		when(request.send()).thenReturn(ethCall);

		when(web3j.platoneCall(any(Transaction.class), eq(DefaultBlockParameterName.LATEST)))
	...
	public void testTransaction() throws Exception {
		...
		assertThat(contract.performTransaction(new Address(BigInteger.TEN),
				new Uint256(BigInteger.ONE)).send(), is(transactionReceipt));
	}
	...
	public void testTransactionFailed() throws Exception {
		...
		prepareTransaction(transactionReceipt);
		contract.performTransaction(new Address(BigInteger.TEN),
				new Uint256(BigInteger.ONE)).send();
	}
	...
	public void testTimeout() throws Throwable {
		...
		TransactionManager transactionManager = getVerifiedTransactionManager(SampleKeys.CREDENTIALS, 1, 1);

		contract = new TestContract(ADDRESS, web3j, transactionManager, new DefaultGasProvider(), 200L);
	...
	public void testStaticGasProvider() throws IOException, TransactionException {
		...
		when(txManager.executeTransaction(any(), any(), any(), any(), any())).thenReturn(new TransactionReceipt());

		contract = new TestContract(ADDRESS, web3j, txManager, gasProvider, 200L);

		Function func = new Function("test", Arrays.<Type>asList(), Collections.<TypeReference<?>>emptyList());
		contract.executeTransaction(func);

		verify(txManager).executeTransaction(eq(BigInteger.TEN), eq(BigInteger.ONE), any(), any(), any());
	}
	...
	public void testInvalidTransactionReceipt() throws Throwable {
		...
		when(getTransactionReceiptRequest.sendAsync()).thenReturn(Async.run(() -> ethGetTransactionReceipt));
	...
	public void testExtractEventParametersWithLogGivenATransactionReceipt() {
		...
		final java.util.function.Function<String, Event> eventFactory = name -> new Event(name, emptyList());

		final BiFunction<Integer, Event, Log> logFactory = (logIndex, event) -> new Log(false, "" + logIndex, "0",
				"0x0", "0x0", "0", "0x" + logIndex, "", "", singletonList(EventEncoder.encode(event)));
		...
		final List<Contract.EventValuesWithLog> eventValuesWithLogs1 = contract
				.extractEventParametersWithLog(testEvent1, transactionReceipt);

		assertEquals(eventValuesWithLogs1.size(), 1);
		assertEquals(eventValuesWithLogs1.get(0).getLog(), logs.get(0));

		final List<Contract.EventValuesWithLog> eventValuesWithLogs2 = contract
				.extractEventParametersWithLog(testEvent2, transactionReceipt);
	...
	private Contract deployContract(TransactionReceipt transactionReceipt) throws Exception {
		...
		return TestContract.deployRemoteCall(TestContract.class, web3j,
				getVerifiedTransactionManager(SampleKeys.CREDENTIALS), gasProvider, "0xcafed00d", encodedConstructor,
				BigInteger.ZERO, 200L).send();
	}

	@SuppressWarnings("unchecked")
	private void prepareEthGetCode(String binary) throws IOException {
		PlatoneGetCode ethGetCode = new PlatoneGetCode();
		ethGetCode.setResult(Numeric.prependHexPrefix(binary));

		Request<?, PlatoneGetCode> ethGetCodeRequest = mock(Request.class);
		when(ethGetCodeRequest.send()).thenReturn(ethGetCode);
		when(web3j.platoneGetCode(ADDRESS, DefaultBlockParameterName.LATEST)).thenReturn((Request) ethGetCodeRequest);
	}

	private static class TestContract extends Contract {
		public TestContract(String contractAddress, Web3j web3j, Credentials credentials, GasProvider gasProvider,
				Long chainId) {
			super(TEST_CONTRACT_BINARY, contractAddress, web3j, credentials, gasProvider, chainId);
		}

		public TestContract(String contractAddress, Web3j web3j, TransactionManager transactionManager,
		...
		public RemoteCall<Utf8String> callSingleValue() {
			Function function = new Function("call", Arrays.<Type>asList(),
		...
		public RemoteCall<TransactionReceipt> performTransaction(Address address, Uint256 amount) {
			Function function = new Function("approve", Arrays.<Type>asList(address, amount),
					Collections.<TypeReference<?>>emptyList());
			return executeRemoteCallTransaction(function);
		}

		public List<EventValues> processEvent(TransactionReceipt transactionReceipt) {
			Event event = new Event("Event", Arrays.<TypeReference<?>>asList(new TypeReference<Address>(true) {
			}, new TypeReference<Uint256>() {
			}));
