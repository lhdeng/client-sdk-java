...
public abstract class Filter<T> {
    ...
    public void run(ScheduledExecutorService scheduledExecutorService, long blockTime) {
        try {
            PlatoneFilter ethFilter = sendRequest();
    ...
    private void getInitialFilterLogs() {
        try {
            Optional<Request<?, PlatoneLog>> maybeRequest = this.getFilterLogs(this.filterId);
            PlatoneLog ethLog = null;
            if (maybeRequest.isPresent()) {
                ethLog = maybeRequest.get().send();
            } else {
                ethLog = new PlatoneLog();
    ...
    private void pollFilter(PlatoneFilter ethFilter) {
        PlatoneLog ethLog = null;
        try {
            ethLog = web3j.platoneGetFilterChanges(filterId).send();
        } catch (IOException e) {
            throwException(e);
        }
        if (ethLog.hasError()) {
    ...
    abstract PlatoneFilter sendRequest() throws IOException;

    abstract void process(List<PlatoneLog.LogResult> logResults);
    ...
    public void cancel() {
        ...
        try {
            PlatoneUninstallFilter ethUninstallFilter = web3j.platoneUninstallFilter(filterId).send();
     ...
     */
    protected abstract Optional<Request<?, PlatoneLog>> getFilterLogs(BigInteger filterId);
